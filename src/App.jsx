...
function AnalysisTab({prices,watchlist,isMobile}) {
  ...
  const [selectedMeta,setMeta]     = useState(null)  // guardamos nombre/símbolo al seleccionar
  const [searchQ,setSearchQ]       = useState('')
  const [showDropdown,setDrop]     = useState(false)
  const {results,searching}        = useGlobalSearch(searchQ,prices)

  // useAnalysis ahora devuelve coinData (precio actual de cualquier moneda)
  const {signal,coinData,loading,error} = useAnalysis(selectedId)

  // La moneda a mostrar: prioridad coinData (siempre actualizado), fallback meta guardado
  ...
  const regimeLabel = r => ({
    strong_bull:'🟢 Tendencia alcista fuerte',
    bull:       '🟡 Tendencia alcista',
    lateral:    '⚪ Mercado lateral',
    bear:       '🟠 Tendencia bajista',
    strong_bear:'🔴 Tendencia bajista fuerte',
    unknown:    '⚪ Sin determinar',
  }[r]||r)

  const divLabel = d => d==='bullish'?'🟢 Divergencia alcista':d==='bearish'?'🔴 Divergencia bajista':'Sin divergencia'

  function selectCoin(p) {
    setSelectedId(p.id)
    setMeta(p)
    setSearchQ('')
    setDrop(false)
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      ...
      {/* Buscador limpio — sin chips de sugerencia */}
      <Card>
        <CT mb={8}>ANÁLISIS TÉCNICO · 12 INDICADORES · TIMEFRAME DIARIO 1D</CT>
        <div style={{fontSize:11,color:DIM,lineHeight:1.9,marginBottom:14}}>
          Escribe el nombre o símbolo de cualquier moneda para analizar.
          Los indicadores usan <span style={{color:WHITE}}>90 días de histórico diario</span> —
          ideal para swing trading de días a semanas.<br/>
          Eficacia combinada: <span style={{color:GOLD}}>~68-72% en tendencias · ~50% en laterales</span>
        </div>

        <div style={{position:'relative'}}>
          <SearchBox
            value={searchQ}
            onChange={v=>{setSearchQ(v);setDrop(true)}}
            busy={searching}
            placeholder="Escribe la moneda a analizar — L3, PEPE, SOL, cualquiera…"
          />
          {/* Dropdown de resultados */}
          {showDropdown&&searchQ&&(results.length>0||searching)&&(
            <div style={{position:'absolute',top:'100%',left:0,right:0,zIndex:30,background:'#181818',border:'1px solid rgba(255,140,0,.2)',borderRadius:8,marginTop:4,overflow:'hidden',maxHeight:220,overflowY:'auto'}}>
              {results.slice(0,10).map(p=>(
                <button key={p.id} onClick={()=>selectCoin(p)}
        ...
        {/* Watchlist rápido — solo si el usuario ya tiene monedas guardadas */}
        {watchlist.length>0&&(
        ...
        <Card>
          <div style={{textAlign:'center',padding:isMobile?'24px 0':'40px 0'}}>
            ...
            <div style={{fontSize:11,color:'rgba(255,255,255,.3)',lineHeight:2}}>
              RSI ajustado + Divergencias · MACD · Bollinger<br/>
              Stoch RSI · Williams %R · ADX · ROC 10d/30d<br/>
              Volumen · OBV · Sesgo de volumen · Soporte/Resistencia
            </div>
          </div>
        </Card>
      )}
      ...
      {selectedId&&loading&&(
        <Card><div style={{textAlign:'center',padding:'28px 0',color:DIM,fontSize:12}}>⟳ Cargando 90 días · calculando 12 indicadores…</div></Card>
      )}
      {selectedId&&error&&(
          ...
          <>
            ...
            <Card accent style={{padding:isMobile?'16px 14px':'20px 24px'}}>
              <div style={{display:'flex',flexDirection:isMobile?'column':'row',alignItems:isMobile?'flex-start':'center',justifyContent:'space-between',gap:14}}>
                <div>
                  <div style={{fontSize:10,letterSpacing:'.2em',color:DIM,marginBottom:8}}>
                    SEÑAL GLOBAL · {displayCoin?.symbol?.toUpperCase()||selectedId.toUpperCase()} · {displayCoin?.name||''}
                  </div>
                  ...
                  <div style={{fontSize:11,color:DIM,marginBottom:8}}>
                    ...
                    Señales: <span style={{color:GREEN}}>{signal.buyCount} compra</span> /
                  ...
                  {/* Precio — usa coinData que siempre trae precio de cualquier moneda */}
                  {displayCoin&&(
                ...
                <div style={{display:'flex',flexDirection:'column',gap:10,minWidth:isMobile?'100%':200}}>
                  {/* Confianza */}
                  <div>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                  ...
                  <div style={{fontSize:11,color:signal.divergence==='bullish'?GREEN:signal.divergence==='bearish'?RED:DIM}}>{divLabel(signal.divergence)}</div>
                </div>
              </div>
              {/* Badges resumen */}
              <div style={{marginTop:12,display:'flex',flexWrap:'wrap',gap:8}}>
                {signal.adx!=null&&<span style={{fontSize:10,padding:'3px 9px',borderRadius:4,background:'rgba(77,184,255,.08)',border:'1px solid rgba(77,184,255,.18)',color:BLUE}}>ADX {signal.adx.toFixed(1)} · {signal.adx>30?'Tendencia fuerte':signal.adx>20?'Moderada':'Lateral'}</span>}
                ...
                {signal.williamsR!=null&&<span style={{fontSize:10,padding:'3px 9px',borderRadius:4,background:'rgba(255,204,0,.06)',border:'1px solid rgba(255,204,0,.15)',color:GOLD}}>W%R: {signal.williamsR?.toFixed(1)}</span>}
              </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:12}}>
              <Card>
                <CT>RSI (14) · Percentil histórico: {signal.rsiPercentile?.toFixed(0)}%</CT>
                ...
                <div style={{fontSize:10,padding:'6px 8px',background:'rgba(255,255,255,.03)',borderRadius:5,color:DIM,lineHeight:1.6}}>
                  Umbral compra: <span style={{color:GREEN}}>&lt;{signal.regime==='strong_bull'?22:signal.regime==='bull'?27:30}</span> · venta: <span style={{color:RED}}>&gt;{signal.regime==='strong_bull'?82:signal.regime==='bull'?78:70}</span><br/>
                  {signal.regime==='strong_bull'||signal.regime==='bull'?'Bull: umbral venta subido, sobrecomprado es normal.':'Umbral estándar.'}
                </div>
              </Card>
              <Card>
                ...
                <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:11,color:DIM,marginTop:2}}>{signal.stochRsi<20?'Sobrevendido extremo':signal.stochRsi>80?'Sobrecomprado extremo':'Neutral'}</div>
                  </div>
                </div>
                {signal.williamsR!=null&&(
                  <div style={{borderTop:'1px solid rgba(255,255,255,.05)',paddingTop:8,marginTop:4}}>
                    <div style={{fontSize:9,color:DIM,marginBottom:3}}>WILLIAMS %R</div>
                    <HBar value={Math.abs(signal.williamsR)} min={0} max={100}
                      color={signal.williamsR<-80?GREEN:signal.williamsR>-20?RED:GOLD}
                      label="" sub={signal.williamsR<-80?'Sobrevendido extremo (<-80)':signal.williamsR>-20?'Sobrecomprado (>-20)':'Zona neutral'}/>
                    <div style={{fontSize:11,fontWeight:700,color:signal.williamsR<-80?GREEN:signal.williamsR>-20?RED:DIM,marginTop:4}}>{signal.williamsR?.toFixed(1)}</div>
                  </div>
                )}
              </Card>
              <Card>
                ...
                <div style={{fontSize:11,fontWeight:700,color:signal.macdHist>=0?GREEN:RED,marginBottom:6}}>{signal.macdHist>=0?'▲ Momentum positivo':'▼ Momentum negativo'}</div>
                {signal.roc10!=null&&<div style={{borderTop:'1px solid rgba(255,255,255,.05)',paddingTop:8,display:'flex',gap:16}}>
                  <div><div style={{fontSize:9,color:DIM,marginBottom:2}}>ROC 10d</div><div style={{fontSize:12,fontWeight:700,color:signal.roc10>0?GREEN:RED}}>{signal.roc10>0?'+':''}{signal.roc10?.toFixed(2)}%</div></div>
                  {signal.roc30!=null&&<div><div style={{fontSize:9,color:DIM,marginBottom:2}}>ROC 30d</div><div style={{fontSize:12,fontWeight:700,color:signal.roc30>0?GREEN:RED}}>{signal.roc30>0?'+':''}{signal.roc30?.toFixed(2)}%</div></div>}
                </div>}
              </Card>
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:12}}>
              <Card>
                ...
                <div style={{fontSize:10,padding:'6px 9px',background:'rgba(255,255,255,.03)',borderRadius:5,color:DIM}}>
                  {displayCoin?.current_price<signal.bollLower
                    ?<span style={{color:GREEN}}>▲ Bajo banda inferior → zona de rebote potencial</span>
                    :displayCoin?.current_price>signal.bollUpper
                    ?<span style={{color:RED}}>▼ Sobre banda superior → posible corrección</span>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  {signal.adx!=null&&(
                    <div>
                      <HBar value={signal.adx} min={0} max={50}
                        ...
                        sub={signal.adx>30?`Tendencia fuerte · ${signal.pdi>signal.mdi?'+DI>-DI alcista':'-DI>+DI bajista'}`:signal.adx>20?'Tendencia moderada':'Lateral — indicadores menos fiables'}/>
                  ...
                  {signal.volRatio!=null&&(
                    <HBar value={Math.min(signal.volRatio*100,250)} min={0} max={250}
                      ...
                      label={`Volumen (${(signal.volRatio*100).toFixed(0)}% del promedio 20d)`}
                      sub={signal.volRatio>2?'Muy alto — movimiento confirmado':signal.volRatio>1.5?'Alto — señal más fiable':signal.volRatio<0.4?'Muy bajo — sin convicción':'Normal'}/>
                  )}
                  {signal.obvTrend&&(
                    <div style={{padding:'8px 10px',background:`rgba(${signal.obvTrend==='up'?'0,210,106':'255,77,77'},.06)`,border:`1px solid rgba(${signal.obvTrend==='up'?'0,210,106':'255,77,77'},.18)`,borderRadius:6}}>
                      ...
                      <div style={{fontSize:10,color:DIM}}>{signal.obvTrend==='up'?'Compradores absorbiendo oferta — señal alcista':'Vendedores liquidando posiciones — señal bajista'}</div>
                    </div>
                  )}
                </div>
              </Card>
...
// ── OVERVIEW — limpio, sin ETH ni "rastreadas" ─────────────────
function OverviewTab({prices,stats,fearGreed,wsStatus,watchlist,onToggle,onChart,isMobile}) {
  ...
  // Solo 2 KPIs — Market Cap y Volumen 24h, más limpios
  const kpis=[
    {label:'MARKET CAP GLOBAL',value:fmt(stats?.total_market_cap?.usd),sub:stats?`BTC dominance ${stats.market_cap_percentage?.btc?.toFixed(1)}% · ETH ${stats.market_cap_percentage?.eth?.toFixed(1)}%`:'…',accent:true},
    {label:'VOLUMEN 24H',value:fmt(stats?.total_volume?.usd),sub:'Global · todas las exchanges'},
  ]
    ...
    <>
      ...
      <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:10,marginBottom:12}}>
        {kpis.map((k,i)=>(
          <Card key={i} accent={k.accent}>
            ...
            <div style={{fontSize:isMobile?20:26,fontWeight:700,color:G,lineHeight:1,marginBottom:4}}>{k.value}</div>
      ...
      <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'2fr 1fr',gap:12,marginBottom:12}}>
        ...
        <Card style={{display:'flex',flexDirection:'column',justifyContent:'space-between'}}>
          <CT>ESTADO DE MERCADO</CT>
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            <div>
              <div style={{fontSize:9,color:DIM,marginBottom:3,letterSpacing:'.1em'}}>FEED EN VIVO</div>
              <WsInd status={wsStatus}/>
            </div>
            <div>
              <div style={{fontSize:9,color:DIM,marginBottom:3,letterSpacing:'.1em'}}>WATCHLIST</div>
              <div style={{fontSize:22,fontWeight:700,color:G}}>{watchlist.length} <span style={{fontSize:12,color:DIM,fontWeight:400}}>monedas</span></div>
            </div>
            <div>
              <div style={{fontSize:9,color:DIM,marginBottom:3,letterSpacing:'.1em'}}>DOM. BTC</div>
              <div style={{fontSize:18,fontWeight:700,color:WHITE}}>{stats?.market_cap_percentage?.btc?.toFixed(1)||'—'}%</div>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:12}}>
        ...
        <Card>
          ...
          {wlCoins.length===0
            ?<div style={{textAlign:'center',padding:'20px 0',color:DIM,fontSize:12}}><div style={{fontSize:26,marginBottom:8}}>☆</div>Añade monedas en la tab Precios</div>
  ...
  return (
    <Card>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:isMobile?'flex-start':'center',flexDirection:isMobile?'column':'row',gap:8,marginBottom:12}}>
        <CT mb={0}>TOP 500 + BÚSQUEDA GLOBAL</CT>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      ...
      <Card>
        ...
        <div style={{marginTop:8,fontSize:11,color:DIM}}>O pulsa ★ en la tab PRECIOS</div>
      </Card>
      {watched.length===0
        ?<Card><div style={{textAlign:'center',padding:isMobile?28:44,color:DIM}}><div style={{fontSize:40,marginBottom:12}}>☆</div><div style={{fontSize:14,color:WHITE}}>Watchlist vacía</div></div></Card>
        :<>
          ...
          {isMobile?<button onClick={onClear} style={{width:'100%',padding:11,background:'rgba(255,77,77,.07)',border:'1px solid rgba(255,77,77,.2)',borderRadius:8,color:RED,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:700}}>✕ LIMPIAR WATCHLIST</button>:(
            <Card>
              ...
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr>{['TOKEN','PRECIO','24H','MKT CAP','QUITAR'].map((h,i)=><th key={i} style={{fontSize:10,color:'rgba(255,140,0,.48)',fontWeight:700,fontFamily:'inherit',paddingBottom:9,borderBottom:'1px solid rgba(255,140,0,.09)',textAlign:i>0?'right':'left'}}>{h}</th>)}</tr></thead>
...
function WalletTab({isMobile}) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      {ok
        ?<div style={{padding:10,background:'rgba(0,210,106,.05)',border:'1px solid rgba(0,210,106,.2)',borderRadius:8,display:'flex',alignItems:'center',gap:8}}><Dot color={GREEN}/><span style={{fontSize:12,color:GREEN,fontWeight:700}}>ZERION CONECTADO ✓ · proxy server-side activo</span></div>
      ...
      {(portfolio||positions.length>0)&&<>
        ...
        {chains.length>1&&<Card>
          <CT>DISTRIBUCIÓN POR RED</CT>
          {chains.map(c=>{const v=byChain[c].reduce((s,p)=>s+p.value,0),p=totalValue>0?(v/totalValue)*100:0,color=CHAIN_COLORS[c?.toLowerCase()]||'rgba(255,255,255,.4)';return(
            <div key={c} style={{marginBottom:10}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}><div style={{display:'flex',alignItems:'center',gap:7}}><ChainBadge chain={c}/><span style={{fontSize:11,color:DIM}}>{byChain[c].length} tokens</span></div><div style={{display:'flex',gap:10}}><span style={{fontSize:11,fontWeight:700,color:WHITE}}>{fmt(v)}</span><span style={{fontSize:10,color:DIM}}>{p.toFixed(1)}%</span></div></div>
              <div style={{height:4,background:'rgba(255,255,255,.06)',borderRadius:2,overflow:'hidden'}}><div style={{width:`${p}%`,height:'100%',background:color,borderRadius:2}}/></div>
            </div>
          )})}
        </Card>}
        ...
        <div style={{position:'relative',zIndex:1,maxWidth:1300,margin:'0 auto',padding:isMobile?'0 12px 80px':'0 24px 56px'}}>
          <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:isMobile?'12px 0 14px':'18px 0 22px',borderBottom:'1px solid rgba(255,140,0,.13)',marginBottom:isMobile?12:18}}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              ...
              <div><div style={{fontSize:isMobile?13:15,fontWeight:700,letterSpacing:'.14em',color:G}}>DEFI PULSE</div><div style={{fontSize:9,color:'rgba(255,140,0,.38)',letterSpacing:'.1em'}}>v9.1 · CEREBRO DeFi</div></div>
            </div>
          ...
          {!isMobile&&<div style={{display:'flex',gap:5,marginBottom:18,overflowX:'auto'}}>
