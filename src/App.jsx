...
function AnalysisTab({prices,watchlist,isMobile}) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:12}}>
      ...
      {selectedId&&loading&&<Card><div style={{textAlign:'center',padding:'28px 0',color:DIM,fontSize:12}}>⟳ Cargando historia + BTC + precio en paralelo…</div></Card>}
      {selectedId&&error&&<Card><div style={{textAlign:'center',padding:'20px 0',color:RED,fontSize:12}}>Error: {error}</div></Card>}
  ...
  return (
    <Card>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:isMobile?'flex-start':'center',flexDirection:isMobile?'column':'row',gap:8,marginBottom:12}}>
        <CT mb={0}>TOP 500 + BÚSQUEDA GLOBAL</CT><WsInd status={wsStatus} short={isMobile}/>
      </div>
      ...
      {loading?<div style={{textAlign:'center',padding:44,color:DIM}}>Cargando top 200…</div>:visible.length===0&&!searching?<div style={{textAlign:'center',padding:36,color:DIM}}>Sin resultados</div>:isMobile?(
